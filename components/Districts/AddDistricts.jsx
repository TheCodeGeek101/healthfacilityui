...
import { districtsFieldConfig, dropIn } from '@/utils/Constants'; // Import districts field config
...
const AddDistricts = ({ setCreateDistrictOpen }) => {
    const district = `${districtsEndpoint}/add`;
    const getDistrict = `${districtsEndpoint}/all`;
    const [loading, setLoading] = useState(false);
    const [errors, setErrors] = useState({});
    const [districtData, setDistrictData] = useState(createInitialFormState(districtsFieldConfig)); // Use districts field config
    const { setDistricts } = useContext(DataContext); // Update to setDistricts

    const handleChange = (e, name) => {
        setDistrictData({ ...districtData, [name]: e.target.value });
        if (errors[name]) {
    ...
    const onSubmit = async (e) => {
        ...
        const validationErrors = validateForm(districtsFieldConfig, districtData);
        ...
        try {
          setLoading(true);
          console.log('client data is:' + JSON.stringify(districtData));
          ...
          if (response.status == 201) {
            ...
            setDistrictData(createInitialFormState(districtsFieldConfig)); // Reset form on successful submission
            setErrors({}); // Clear any errors
    
            // Fetch the data from the server to update the context
            const fetchResponse = await axios.post('/api/GetData', {
              endPoint: getDistrict,
            });
            console.log('error check:' + fetchResponse.data);
            if (fetchResponse.status === 200) {
              toast.success(`Districts retrieved successfully!`);
              setDistricts(fetchResponse.data); // Replace existing clients with new list
                ...
                >
                    ...
                    <div className="relative flex flex-col">
                        ...
                        <form onSubmit={onSubmit}>
                            {districtsFieldConfig.map(({ label, name, placeholder, type }) => ( // Use districts field config
